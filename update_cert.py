...
def oneview_get_current_cert_info(host, session_id):
    ...
    headers = {"X-Api-Version": "7600", "auth": session_id}
...
def oneview_cert_needs_replacement(ov_host, cert_info):
    ...
    if delta > five_years:
        print(
            f"OneView cert on {ov_host} validUntil {valid_until} > 5 years: likely default cert, needs replacement."
        )
        return True
    if delta < thirty_days:
        print(
            f"OneView cert on {ov_host} validUntil {valid_until} < 30 days: cert nearing expiry, needs replacement."
        )
        return True
    print(
        f"OneView cert on {ov_host} validUntil {valid_until} is valid, no replacement needed."
    )
    return False


def oneview_request_csr(
    host,
    session_id,
    common_name,
    alt_names,
...
def create_certificate_request(api_instance, namespace, name, csr_pem_base64):
    ...
    try:
        api_instance.create_namespaced_custom_object(
            "cert-manager.io", "v1", namespace, "certificaterequests", body
        )
    except ApiException as e:
        if e.status == 409:  # Already exists
            api_instance.delete_namespaced_custom_object(
                "cert-manager.io", "v1", namespace, "certificaterequests", name
            )
            time.sleep(2)
            api_instance.create_namespaced_custom_object(
                "cert-manager.io", "v1", namespace, "certificaterequests", body
...
def main():
    ...
    ilo_map = dict(
        item.split("=", 1)
        for item in os.environ.get("ILO_CERT_MAP", "").split(",")
        if item
    )
    ...
    oneview_map = dict(
        item.split("=", 1)
        for item in os.environ.get("ONEVIEW_CERT_MAP", "").split(",")
        if item
    )
    ...
    for ov_host, host_name in oneview_map.items():
        try:
            ...
            if oneview_cert_needs_replacement(cert_info):
                ...
                fqdn = (
                    f"{host_name.strip()}.{os.environ.get('DNS_SUFFIX', 'example.com')}"
                )
                ...
                create_certificate_request(
                    api_instance, namespace, host_name.strip(), csr_base64
                )

                # Wait for cert to be issued
                full_cert_pem = wait_for_certificate(
                    api_instance, namespace, host_name.strip()
                )
